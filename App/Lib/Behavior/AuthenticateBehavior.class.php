	...
	public function run(&$params) {
		...
		if (in_array($a, $permission)) {
			...
			if (in_array($a, $allow)) {
                ...
                $position_ids =  explode(',', session('extra_position'));
                array_unshift($position_ids, session('position_id'));
				$ask_per = M('permission')->where(['url'=>$url, 'position_id'=>['in', array_unique($position_ids)]])->find();

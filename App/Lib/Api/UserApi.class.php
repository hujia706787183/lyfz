...
class UserApi extends ApiBase {
    ...
    */
    public function login(){
        if ($this->isPost()){
            ...
            $m_user = M('user');
            $user = $m_user->alias('user')->join(C('DB_PREFIX').'role role ON user.user_id=role.user_id')->where(array('name' => trim($username)))->find();

            if ($user['password'] == md5(md5(trim($password)).$user['salt'])){
                if (-1 == $user['status']) {
                    $this->ajaxReturn('',2, 'error');
                } elseif (0 == $user['status']) {
                    $this->ajaxReturn('',2, '该账号未激活');
                }elseif($user['status'] == 2){
                    $this->ajaxReturn('',2, '该账号已停用');
                }elseif (35 == $user['position_id']) {
                    $this->ajaxReturn('',2, '员工已离职');
                }else{
                    $d_role = D('RoleView');
                    $role = $d_role->where('user.user_id = %d', $user['user_id'])->find();
                    if (!is_array($role) || empty($role)) {
                        $this->ajaxReturn('',2,'此账号不存在');
                    } else {
    ...
    */
    ...
    /**
     * 添加获取未离职且激活状态用户列表
     * @param int $all 默认不包括离职和状态未激活的
     */
    public function getRoleList($all = 0){
        if ($all == 0){
            $where['user.status'] = 1;
            $where['department.department_id'] = ['neq', 13];
        }
        $roleList = M('user')->alias('user')
    ...
    {
        if (!$token){
            $this->ajaxReturn(null, -1000, '未登录');
        }
        if($role = M('user')->alias('user')->join(C('DB_PREFIX').'role role ON user.user_id=role.user_id')->where(['token'=>$token])->find()){
            if ($role['position_id'] != 89) { // 不为异业合作的显示所有
                $role['mobile_permission'] = [
                    ['name' => 'reports', 'description' => '工作报告', 'permission' => false],
                    ['name' => 'customer', 'description' => '订单客户', 'permission' => false],
                    ['name' => 'count', 'description' => '统计', 'permission' => false],
                    ['name' => 'createCustomerMain', 'description' => '创建客户', 'permission' => false],
                    ['name' => 'CustomerInformation', 'description' => '客户资料', 'permission' => false],
                    ['name' => 'service', 'description' => '上门服务', 'permission' => false],
                    ['name' => 'rank', 'description' => '排行榜', 'permission' => false],
                    ['name' => 'work', 'description' => '派工管理', 'permission' => false],
                ];
            } else {
                $role['mobile_permission'] = [
                    ['name'=>'reports', 'description'=>'工作报告', 'permission'=>true],
                    ['name'=>'customer', 'description'=>'订单客户', 'permission'=>true],
                    ['name'=>'count', 'description'=>'统计', 'permission'=>true],
                    ['name'=>'createCustomerMain', 'description'=>'创建客户', 'permission'=>false],
                    ['name'=>'CustomerInformation', 'description'=>'客户资料', 'permission'=>false],
                    ['name'=>'service', 'description'=>'上门服务', 'permission'=>true],
                    ['name'=>'rank', 'description'=>'排行榜', 'permission'=>true],
                    ['name'=>'work', 'description'=>'派工管理', 'permission'=>true],
                ];
            }
            $this->ajaxReturn($role);
        }else{
            $this->ajaxReturn(null, -1, 'token出错');
        }
    }
}
...
class ContactsAction extends Action {
	...
	public function add(){
		if ($_GET['r'] && $_GET['module'] && $_GET['id']) {
			...
			if ($contacts->id_number){
				if(strlen($contacts->id_number)!==18){
					$this->error('身份证长度不正确');
				}
			}
			...
			$this->refer_url=$_SERVER['HTTP_REFERER'];
			$this->customer = $customer->where('customer_id = %s', $_GET['redirect_id'])->find();
	...
	public function edit(){
		...
		if ($this->isPost()) {
			...
			if ($m_contacts->id_number){
				if(strlen($m_contacts->id_number)!==18){
					alert('error', '身份证长度不正确', $_SERVER['HTTP_REFERER']);
				}
			}
	...
	public function index(){
		...
		if(trim($_GET['act']) == 'excel'){
			...
			if($_GET['listrows']){
				...
				$contactsList = $d_contacts->where($where)->order($order)->page($p.','.$listrows)->select();
	...
	public function mDelete(){
		if($_GET['r'] && $_GET['id'] && $_GET['module_id']){
			$m_r = M($_GET['r']);
			$condition = [
				"contacts_id" => I('get.id'),
				"customer_id" => I('get.module_id')
			];
			if($m_r->where($condition)->delete()){
	...
	public function excelExport($contactsList){
		...
		foreach ($list as $k => $v) {
			...
			$objActSheet->setCellValue('E'.$i, $v['qq_no']);
	...
	public function radioListDialog(){
		...
		if($id){
			$contacts_id = $rcc->where('customer_id = %d', $id)->getField('contacts_id', true);
	...
	public function changeDialog(){
		if($this->isAjax()){
			...
			if ($_REQUEST["field"]) {
				$field = trim($_REQUEST['field']) == 'all' ? 'name|telephone|email|address|post|department|description' : $_REQUEST['field'];
				$search = empty($_REQUEST['search']) ? null : trim($_REQUEST['search']);
				$condition = empty($_REQUEST['condition']) ? 'is' : trim($_REQUEST['condition']);
				if	('create_time' == $field || 'update_time' == $field) {
					$search = is_numeric($search)?$search:strtotime($search);
				}

	...
	public function index(){
		...
		foreach($action_log as $k=>$v){
			...
			if($v['module_name'] == 'log'){
				...
				$appHtml = '<pre style="margin-top:8px">'.$apContent.'</pre>';
			...
			if($v['module_name'] == 'log'){
				...
				$action_name = $action_name == 'add' ? 'view' : $action_name; 
			}

			$action_log[$k]['dynamic'] = $username.'&nbsp;'.$operation.$module.' - <a href="./index.php?m='.$v[module_name].'&a='.$action_name.'&'.$param_name.'&id='.$v[action_id].'">'.$name.'</a>';

			$action_log[$k]['appHtml'] = $appHtml;
			$action_log[$k]['link'] = './index.php?m='.$v[module_name].'&a=view&'.$param_name.'&id='.$v[action_id];
			$action_log[$k]['appHtml_len'] = utf8_strlen($appHtml);
			...
			$action_log[$k]['address'] = $object_module['address'];
			$action_log[$k]['log'] = $object_module['log'];
			$action_log[$k]['x'] = $object_module['x'];
			$action_log[$k]['y'] = $object_module['y'];
		}
		$data =M('user') ->where('role_id =%s',session('role_id')) ->getField('img');

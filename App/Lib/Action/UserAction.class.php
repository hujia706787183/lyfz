...
use App\Utils\Wechat\Enterprise\Application;
/**
 * User Related
 * 用户相关模块
 *
...
class UserAction extends Action {
	...
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('logout','role_ajax_add','getrolebydepartment','dialoginfo','edit', 'listdialog', 'mutilistdialog', 'getrolelist', 'getpositionlist',  'weixin','changecontent','getdepartmentlist', 'role', 'stocks')
		);
		...
		if (session('?name')){
           ...
           //点击按钮之后
			...
			if((!isset($_POST['name']) || $_POST['name'] =='')||(!isset($_POST['password']) || $_POST['password'] =='')){
				...
				$m_user = M('user');
				$user = $m_user->alias('user')->join(C('DB_PREFIX').'role role ON user.user_id=role.user_id')->where(array('name' => trim($_POST['name'])))->find();
				...
				if ($user['password'] == md5(md5(trim($_POST['password'])) . $user['salt'])) {				
					if (-1 == $user['status']) {
					...
					}elseif (35 == $user['position_id']) {
                        alert('error', '员工已离职！');
                    }elseif (89 == $user['position_id']) {
                        alert('error', '无权登录！');
                    }else {
						$d_role = D('RoleView');
						...
						if (!is_array($role) || empty($role)) {
                            ...
                            session('extra_position', $user['extra_position']);

							$token = M('user')->where('user_id = %d', $user['user_id'])->getField('token');
							if (empty($token)) {
								$token = md5(md5($user['user_id']).time());
								M('user')->where('user_id = %d', $user['user_id'])->setField('token', $token);
							}
							...
							if (session('?user_url')){//检测当前session中是否含有user_url
                                alert('success', L('LOGIN_SUCCESS'), session('user_url'));
                                session('user_url', null);//user_url只能使用一次
                            }else{
	...
	public function listDialog() {
		...
		$where = 'user.status = 1';
		$role_list = $this->role_list = $d_role_view->where($where)->limit('0,10')->select();
		
		$count =  $d_role_view->where($where)->count();
		$this->count_num = $count;  //修改bug count改成count_num
...
//		$department_id = M('position')->where('position_id = %d', session('position_id'))->getField('department_id');
		$department_id = M('position')->getField('department_id');
	...
	public function edit(){
		if ($this->isPost()) {
			...
			if ($m_user->create()) {
				if(isset($_POST['password']) && $_POST['password']!=''){
				    if (strlen($_POST['password']) < 6)
                        alert('error',"密码不能少于六位数", U('user/edit'));
					$m_user->password = md5(md5(trim($_POST["password"])) . $user['salt']);
				} else {
					unset($m_user->password);
				}
				$m_user->extra_position = implode(',', I('post.extra_position'));
				// $is_update = false;
                // if(session('?admin')){
				...
				if($m_user->save() || $is_update){
					if($_POST['user_id'] == intval(session('user_id'))){
						session('name', $_POST['name']);
						if($upload) F('img_id_'.$_POST['user_id'],$upload);
					}
			...
			// $user['extra_position'] = explode(',', $user['extra_position']);
			$department_list = getSubDepartment(0, M('role_department')->select());
			$this->assign('department_list', $department_list);
			$positions_list = M('position')->field('position_id,name')->select();
    ...
    public function edit_staff(){
        if ($this->isPost()) {
            ...
            $id_card = new \App\Extend\IDCard(I('post.id_number'));
            if (!$id_card->validate())  $this->error('身份证号输入错误');
            if ($m_staff->where(['id_number'=>I('post.id_number'), 'id'=>['neq', $staff_id]])->find())  $this->error('身份证号已存在');// 编辑员工时将自身排除

            if ($m_staff->where('id=%d',$staff_id)->save($_POST)!==false) {
                if ($_POST['dimisson']==2){
                    $user_id = $m_staff->getFieldById($staff_id,'user_id');
                    M('user')->where('user_id = %d', $user_id)->save(['status'=>2]);//将员工状态改为离职，对应的用户状态也改为离职状态
                }
                alert('success',L('EDIT_USER_INFO_SUCCESS'),U('user/edit_staff','id='.$_POST['id']));
            } else {
                alert('error',L('EDIT_USER_INFO_FAILED'),$_SERVER['HTTP_REFERER']);
            }
        }else{
            ...
            $this->r_url = $_SERVER['HTTP_REFERER'];
            $staff_id = $_GET['id'] ;
            $staff = M('Staff')->where('id = %d', $staff_id)->find() ;
            $position_list = M('position')->where('department_id = %d', $staff['department'])->select();
...
//            $where['role.role_id'] = array('in', $departRoleIdArr);
            $where['user.name'] = array('like', '%'.trim($_GET['name']).'%');
            if($_GET['department'] != 'all'){
                $where['position.department_id'] = $this->_get('department');
            }
//            $list = $d_role_view -> where($where) ->order('role_id')->page($p.',10')->select();

            $list = $d_role->alias('role')
                ->field('role.user_id,role.role_id role_id,role.position_id,
                user.name user_name,user.status status,user.weixinid,user.category_id,user.sex,user.address,user.email,user.img,user.telephone,user.dashboard,
                position.name role_name,position.parent_id,position.department_id,position.description,
                role_department.name department_name,
                convert(user.name using gbk) user_name_gbk')
                ...
                ->where($where)
                ->order('position.department_id,user_name_gbk')
                ->page($p.',10')
                ->select() ;
            $data['list'] = $list;
	...
	*/
		...
		if ($this->isPost()){
			...
			if($_POST['radio_type'] == 'email'){
				...
				if (!isset($_POST['name']) || $_POST['name'] == '') {
                    ...
                    alert('error', '密码不能少于六位数', $_SERVER['HTTP_REFERER']);
                } elseif (!isset($_POST['category_id']) || $_POST['category_id'] == ''){
					alert('error', L('PLEASE_SELECT_USER_CATEGORY'), $_SERVER['HTTP_REFERER']);
				} elseif (!session('?admin') && intval($_POST['category_id'])==1) {
					alert('error', L('YOU_HAVE_NO_PERMISSION_TO_ADD_ADMIN'), $_SERVER['HTTP_REFERER']);
    ...
    public function add_staff(){
        ...
        $m_staff = M('Staff');
        if ($this->isPost()){
            $staffData = $m_staff->create();
           //是否填写必填内容
            if (!isset($_POST['name']) || $_POST['name'] == '') {
                alert('error', L('INPUT_USER_NAME'), $_SERVER['HTTP_REFERER']);
            } elseif (!isset($_POST['personal_call']) || $_POST['personal_call'] == ''){
                alert('error', L('INPUT_PERSONAL_CALL'), $_SERVER['HTTP_REFERER']);
            }elseif (!isset($_POST['address']) || $_POST['address'] == ''){
                alert('error', L('INPUT_ADDRESS'), $_SERVER['HTTP_REFERER']);
            }elseif (!isset($_POST['id_number']) || $_POST['id_number'] == ''){
                alert('error', L('INPUT_ID_NUMBER'), $_SERVER['HTTP_REFERER']);
            }elseif (!isset($_POST['date_board']) || $_POST['date_board'] == ''){
                alert('error', L('INPUT_DATE_BOARD'), $_SERVER['HTTP_REFERER']);
            }
            $id_card = new \App\Extend\IDCard(I('post.id_number'));
            if (!$id_card->validate())  $this->error('身份证号输入错误');
            if ($m_staff->where(['id_number'=>I('post.id_number')])->find())  $this->error('身份证号已存在');

            if (I('post.whether_create_user')){
                //为用户设置默认导航（根据系统菜单设置中的位置）
                $m_navigation = M('navigation');
                $navigation_list = $m_navigation->order('listorder asc')->select();
                $menu = array();
                ...
                $navigation = serialize($menu);
                $salt = substr(md5(time().rand(1000,9999)),0,4);
                $userData = [
                    'name' => I('post.name'),
                    'password' => md5(md5('123443211') . $salt),//密码默认为123443211
                    'category_id' => 2,//默认为员工
                    'department_id' => I('post.department'),
                    'position_id' => I('post.position'),
                    'status'  => 1,
                    'salt' => $salt,
                    'navigation' => $navigation,
                    'sex' => I('post.sex'),
                    'email' => I('post.personal_email'),
                    'telephone' => I('post.personal_call'),
                    'address' => I('post.address'),
                    'reg_time' => time(),
                    'reg_ip' => $_SERVER["REMOTE_ADDR"],
                ];
                $m_user = M('User');
                if ($user = $m_user->where(['name'=>I('post.name')])->find()){
                    ...
                    $m_user->where('user_id = %d', $user_id)->setField('role_id', $role_id);
                    actionLog($user_id);
                    $staffData['user_id'] = $user_id;
                }
            }

            if($re_id = $m_staff->add($staffData)){//可以得到插入的id
                if($_POST['submit'] == L('ADD')){
                    alert('success', L('ADD_USER_SUCCESS_USER_CAN_LOGIN_NOW'), U('user/staffIndex'));
                }else{
                    alert('success', L('ADD_USER_SUCCESS_USER_CAN_LOGIN_NOW'), U('user/add_staff'));
                }
            }else{
                alert('error', L('ADDING FAILS CONTACT THE ADMINISTRATOR' ,array('')),$_SERVER['HTTP_REFERER']);
            }
    ...
    public function view_staff()
    {
        ...
        $m_staff = M('Staff') ;
        $staff = $m_staff->where('id = %d', $staff_id)->alias('staff')
            ->join(C('DB_PREFIX').'role_department role_department  ON role_department.department_id=staff.department')
           ->join(C('DB_PREFIX').'position position ON position.position_id=staff.position')
               // position.name as position, //后期连position表
            ->field('staff.*,                     
                     role_department.name as department,position.name as position')
            ->find();
        $file_ids = M('rFileStaff')->where('staff_id = %d', $staff_id)->getField('file_id', true);
        $staff['file'] = M('file')->where('file_id in (%s)', implode(',', $file_ids))->select();
        $file_count = 0;
        foreach ($staff['file'] as $key=>$value) {
            $staff['file'][$key]['owner'] = D('RoleView')->where('role.role_id = %d', $value['role_id'])->find();
            $file_count++;
        }
        $staff['file_count'] = $file_count;
        $this->alert = parseAlert();
        $this->staff = $staff ;
        $this->display();
	...
	public function index(){
		...
		$where['status'] = $status;
		// $where['department_id'] = ['neq', 13]; // 去掉离职员工
		$where['IFNULL(position.department_id,0)'] = ['neq', 13]; // 去掉离职员工
		...
		$user_list = $d_user->order('reg_time desc')->where($where)->page($p.',15')->select();
    ...
    public function staffIndex(){
        $p = isset($_GET['p']) ? intval($_GET['p']) : 1 ;
        $dimisson = isset($_GET['dimisson']) ? intval($_GET['dimisson']) : 1 ;
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
        $search = isset($_GET['search'])?$_GET['search']:'' ;
	    $staff = M('staff') ;
        ...
        import('@.ORG.Page');// 导入分页类
        $where = array() ;
        if ($search) $where['staff.name'] = array('like',"%{$search}%");
        $where['staff.dimisson'] = $dimisson;
        $count = $staff->alias('staff')
            ->where($where)
            ->join(C('DB_PREFIX').'role_department role_department ON role_department.department_id=staff.department')
            ->join(C('DB_PREFIX').'position position ON position.position_id=staff.position')
            ->count();
        $Page = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
        $Page->parameter = "id=".$id.'&dimisson=' . $dimisson;
        $show  = $Page->show();// 分页显示输出
        $staff_list_sql = $staff->alias('staff')
            ->where($where)
            ->join(C('DB_PREFIX').'role_department role_department ON role_department.department_id=staff.department')
			->join(C('DB_PREFIX').'position position ON position.position_id=staff.position')
			->join('left join '.C('Db_PREFIX').'r_file_staff file_staff ON file_staff.staff_id=staff.id')
			->field('staff.*,role_department.name department_name,position.name position_name, count(file_staff.id) as file_count')
			->group('staff.id')
			->buildSql();

		$staff_list = M()->table($staff_list_sql . ' t ')->order('id desc')->page($p.',15')->select();

        $this->assign('staff_list',$staff_list) ;// 赋值数据集
        $this->assign('page',$show);// 赋值分页输出
        $category = M('user_category');
        $this->categoryList = $category->select();
        $this->alert = parseAlert();
	...
	public function weixinbinding(){
		if($_POST['submit']){
			...
			$config = [
				'base' => [
					'app_id' => C('WX_ENTERPRISE_CORPID'),
					'secret' => C('WX_ENTERPRISE_CORPSECRIT'),
				],
				'oauth' => [
					'agentid' => '1000028',
					'scope'   => 'snsapi_base',
					'callback' => '/user/weixinbinding',
				],
			  ];
    ...
    public function getDepartmentList()
    {
        ...
        $departmentList = $department ->cache(true,3600) -> order('convert(name using gbk)') -> select();
        $this -> ajaxReturn($departmentList, '', 1);
	}

    public function stocks()
    {
        $role_id = I('get.role_id');
        $records = M('stocksRecord')->alias('records')
            ->field('user.name, records.*,customer.name customer_name')
            ->join(C('DB_PREFIX').'user user ON user.role_id=records.role_id')
            ->join(C('DB_PREFIX').'r_order_product order_product ON order_product.id=records.type_id and records.type="OrderProduct"')
            ->join(C('DB_PREFIX').'customer customer ON customer.customer_id=order_product.customer_id')
            ->where(['records.role_id'=> $role_id])->select();
        $recordsTotal = array_sum(array_column($records, 'stocks'));
        $this->assign('records', $records);
        $this->assign('recordsTotal', $recordsTotal);
        $this->display();
	}
}
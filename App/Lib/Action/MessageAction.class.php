...
class MessageAction extends Action{
    ...
    public function send(){
        if($_POST['submit']){
            ...
            $to_role = is_array($to_role)?$to_role:[$to_role];

            foreach($to_role as $k=>$v){
                sendMessage($v, $_POST['content']);
            }

            $weixin_user_list = M('user')->where(['user_id'=>['in',$to_role]])->getField('weixinid',true);

            import('@.ORG.WeixinEnterprise');
            $weixin  = new WeixinEnterprise(C('WX_ENTERPRISE_CORPID'), C('WX_ENTERPRISE_CORPSECRIT'));
            $weixin->sendTextMessage('user', array_filter($weixin_user_list), $content);
    ...
    public function tips(){
        ...
        //合同
        $new_num[contract_count] = $m_contract->where("owner_role_id = %d and is_deleted <> 1 and $temp_time >= end_date and end_date >= $current_time ", session('role_id'))->count();

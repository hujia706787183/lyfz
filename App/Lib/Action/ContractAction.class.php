...
class ContractAction extends BaseAction {
	...
	public function add(){
		$contract_custom = C('contract_custom');//M('config') -> where('name="contract_custom"')->getField('value');
		if(!$contract_custom)  $contract_custom = 'NO';
		if($this->isPost()){
			$contract = M('contract');
	...
	public function index(){
		...
		switch ($_GET['by']){
			...
			case 'alert' :
				...
				$where['contract.end_date'] = array('between',$current_time.','.$temp_time);
				break;
			default: 

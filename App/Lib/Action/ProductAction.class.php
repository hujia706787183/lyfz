...
class ProductAction extends Action {
	use App\Lib\Abstracts\ControllerModuleSupport;
...
//	}
	...
	public function _initialize(){
		...
		$action = array(
			...
			'allow'=>array('listdialog','changecontent','adddialog','editdialog', 'mdelete','allproductdialog','validate','check','delimg','sortimg','mutildialog','getcurrentstatus','product_list','getproductids','getproductlist','getproducttype', 'inventory')
		);
		...
		if ($this->isAjax()) {
			...
			foreach($name_list as $k=>$v){
				...
				foreach($result_array as $k2=>$v2){
					if(strpos($v, $v2) > -1){
						$v = str_replace("$v2", "<span style='color:#ff1a37;'>$v2</span>", $v, $count);
						$search += $count;
					}
				}
    ...
    public function index(){
		...
		if ($_GET['category_id']) {
			...
			if(trim($_GET['act']) == 'excel'){
			  ...
			  if($_GET['category_id'] == 9){
                 $product  =M('Product');
                 $inventory = M('InventoryRecords');
			       $category_id =7;
			       $conut = $product->where(['category_id'=>$category_id])->count();
                  $inve =  $inventory->alias('inventory')
                      ->join(C('DB_PREFIX').'product product on  product.product_id = inventory.product_id')
                      ->join(C('DB_PREFIX').'user user on user.user_id = inventory.user_id','right')
                      ->field(
                           'user.name as  user_name,
                            product.name as  name,
                            inventory.out_put as out_put,
                            inventory.inventory_sum as sum,
                            inventory.description as description,
                            inventory.create_time as  create_time
                            '
                      )
                      ->select();
                  $Page = new Page($conut, $listrows);// 实例化分页类 传入总记录数和每页显示的记录数
                  if (!empty($_GET['category_id'])) {
                      $params['category_id'] = 'category_id='.trim(7);
                  }

                  $Page->parameter = implode('&', $params);
                  $paging = $Page->show();// 分页显示输出
              }

			$count = $product->where($where)->count();

			$list = $product->order('sort asc,product_id desc')->where($where)->Page($p.','.$listrows)->select();
		...
		$this->categoryList = getSubCategory(0, $category_list, ''); //类别选项
          //dump($show);exit;
        $this->inven_array = getIndexFields('inventory');
		$this->inven_list = getMainFields('inventory');
...
//			$this->categoryList = $categoryList;
    ...
    public function product_list(){
        ...
        if ($search=I('get.',null)){
            if (I('get.submit')=='search1'){
                $field = I('get.field') ;
                if ($field == 'product_name'){
                    $product_id = I('condition');
                    $where['order_product.product_id'] = $product_id;
                }elseif ($field == 'product_type'){
                    $category_id = I('condition');
                    $productIds = $this -> getProductIds($category_id);
                    $where['order_product.product_id'] = ['in', $productIds];
                }
            }else{
                if ($unfilled_order = I('get.unfilled_order','', 'intval')){
                    $productIds = $this -> getProductIds($unfilled_order);
                    $where['order_product.product_id'] = ['in', $productIds];
                    $where['express.express_number'] = ['exp', 'is null'];
                    $this->assign('unfilled_order1', json_encode($unfilled_order));
                }
            }
        }
        ...
        $res = $db_order_product->alias('order_product')
			...
			->join(C('DB_PREFIX').'express express ON express.order_product_id = order_product.id')
            ...
            ->field(
                'order_product.product_name,
                 ...
                 receivables.price,
                 receivables.description order_description,
                 customer_data.description,
                 express.express_number,
                 express.express_company,
                 sum(receiving.money) as received')
            ->where($where)
...
//            $express_number = M('express')->where('order_product_id=%d', $value['order_product_id'])->order('express_id desc')->getField('express_number');
//            if ($express_number)
//                $value['express_number'] = $express_number;
//            else
//                $value['express_number'] = M('r_order_product_extra')->where('extra_key="express_number" AND order_product_id=%d', $value['order_product_id'])->order('id desc')->getField('extra_value');
//        }
        $product_list = M('product')->field('product_id, category_id, name')->select();
        $product_type = M('product_category')->field('category_id,  name')->select();
    ...
    public function getProductIds($category_id)
    {
        ...
        if (is_array($category_id)){
            $product_list = M('product')->where(['category_id' => ['in', $category_id]])->select();
        }else{
            $product_list = M('product')->where('category_id = %d', $category_id)->select();
        }
   ...
   public function inventory()
   {
       ...
       if ($_REQUEST["field"]) {
           ...
           if (trim($_REQUEST['field']) == "all") {
               $field = is_numeric(trim($_REQUEST['search'])) ? 'product.name|description|user.name' : 'product.name|description|user_name';
                 dump($field);
           } else {
               $field = trim($_REQUEST['field']);
           }
           ...
           $condition = empty($_REQUEST['condition']) ? 'is' : trim($_REQUEST['condition']);
            //field查出时间
           $field_date = M('Fields')->where('(is_main=1 and model="inventory" and form_type="datetime") or(is_main=1 and model="" and form_type="datetime")')->select();
           foreach($field_date as $v){
               if	($field == $v['field']) $search = is_numeric($search)?$search:strtotime($search);
           }
        ...
        $inve =  $inventory->alias('inventory')
            ...
            ->order('inventory_id desc')->where($where)->select();
        $count =  $inventory->alias('inventory')
            ->join(C('DB_PREFIX').'product product on  product.product_id = inventory.product_id')
            ->join(C('DB_PREFIX').'user user on user.user_id = inventory.user_id','right')
            ->field(
                'user.name as  user_name,
                            ...
                            inventory.create_time as  create_time
                            '
            )->order('inventory_id desc')->where( $where)->count();
        $Page = new Page($count, $listrows);// 实例化分页类 传入总记录数和每页显示的记录数
        if (!empty($_GET['category_id'])) {
            $params['category_id'] = 'category_id=' . trim(7);
        }
   ...
   public function output(){
        /// dump(I('post.'));
       $inventory = M('InventoryRecords');
       $inventory->create(I('post.'));

       $sum = $inventory->where(['product_id'=>I('post.product_id'),'user_id'=>I('post.user_id')])->order('inventory_id desc')->limit(1)->getField('inventory_sum');
       
       $inventory->out_put = I('post.mun') * I('post.inve_id');
       $inventory->inventory_sum = $sum + $inventory->out_put;
    ...
    public function invendelete()
    {

        if ($this->isPost()) {
            $inventory = M('InventoryRecords');
            $inventory_ids = is_array($_POST['inventory_id']) ? implode(',', $_POST['inventory_id']) : '';
            if ('' == $inventory_ids) {
                alert('error', L('YOU_HAVE_NOT_CHOOSE_ANY_CONTENT'), $_SERVER['HTTP_REFERER']);
            } else {

                $ids= I('post.inventory_id');

                $ini['inventory_id']=array('in',$ids);

                $n = D('InventoryRecords')->where($ini)->delete();
                ;
                if($n){
                    $this->success('删除成功',U('product/inventory'),3);
                }else{
                    $this->error('删除失败',U('product/inventory'),3);
                }
                exit();
            }
        }

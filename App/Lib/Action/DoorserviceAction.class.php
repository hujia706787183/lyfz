...
class DoorserviceAction extends Action {
    protected $pageSize = 100;
    public function _initialize(){
        header("Content-type: text/html; charset=utf-8");
        $action = array(
            'permission'=>array('getproblemlist','postanswer', 'sendmessagecron'),
            'allow'=>array('lookupdepartmentstaff', 'getsign')
        );
    ...
    public function search(){
        ...
        $params['system_visit'] = I('param.system_visit');
        ...
        if ($param['return_status']==1){
            $where['return_status'] = ['in', [0,1,2]]; //1未联系 2联系不上 0 默认值
        ...
        switch ($a) {
            case '1':
                ...
                switch ($key) {
                    ...
                    // 首要联系人
                    case 'contacts_name':
                        $where['contacts.name'] = array('LIKE',"%{$search}%");
                        break;
                    // 首要联系人电话
                    case 'contacts_telephone':
                        $where['contacts.telephone'] = array('LIKE',"%{$search}%");
                        break;
                    // 上门老师
                    case 'operator_name':
                ...
                $count = $mod->alias('onsiteservice')
                    ->field('onsiteservice.*,customer.name customer_name,contacts.name contacts_name, contacts.telephone contacts_telephone')
                ...
                $list = $mod->alias('onsiteservice')
                    ->field('onsiteservice.*,customer.name customer_name,contacts.name contacts_name, contacts.telephone contacts_telephone')
                    ->join(C('DB_PREFIX').'customer customer ON customer.customer_id=onsiteservice.customer_id')
                    ->join(C('DB_PREFIX').'contacts contacts ON contacts.contacts_id=customer.contacts_id')->order('addtime desc')->where($where)->page($p, $listrows)->select();
            ...
            case '2':
                ...
                $list = $mod->alias('onsiteservice')
                    ...
                    ->join(C('DB_PREFIX').'contacts contacts ON contacts.contacts_id=customer.contacts_id')
                    ->order('addtime desc')
                    ->where($where)->page($p, $listrows)->select();
                $this->assign('page',$show);// 赋值分页输出
                $this -> assign('list',$list);
            ...
            case '3':
                ...
                switch ($key) {
                    ...
                    // 上门老师
                    case 'operator_name':
                        $where['onsiteservice.operator_name'] = array('LIKE',"%{$search}%");
                        break;
                    // 销售人员
                    case 'saleman':
                        $where['onsiteservice.saleman'] = array('LIKE',"%{$search}%");
                        break;
                    // 提交人
                    case 'submitter':
                        $where['onsiteservice.submitter'] = array('LIKE',"%{$search}%");
                        break;
                }
                $department_ids = \App\Lib\Model\User::getDepartmentIds(session('department_id'));
                $role_ids = M('user')->alias('user')
                    ...
                    ->where(['position.department_id'=>['in', $department_ids]])
                    ->field('user.role_id')
                    ->select();
                $role_ids = array_column($role_ids, 'role_id'); // 获取员工所在部门所有的id
                $where['tid'] = ['in', $role_ids];

                if ($param['system_visit']==1){
                    $where['onsiteservice_answer.answer'] = ['exp', 'is null']; //1系统未回访 2系统已回访 0全部
                }elseif ($param['system_visit']==2){//已回访
                    $where['onsiteservice_answer.answer'] = ['exp', 'is not null'];
                }else{
                    unset($where['onsiteservice_answer.answer']);
                }

                import('@.ORG.Page');// 导入分页类
                $count = $mod->alias('onsiteservice')
                    ->field('onsiteservice.*,customer.name customer_name, contacts.name contacts_name, contacts.telephone contacts_telephone,onsiteservice_answer.answer')
                    ->join(C('DB_PREFIX').'customer customer ON customer.customer_id=onsiteservice.customer_id')
                    ->join(C('DB_PREFIX').'contacts contacts ON contacts.contacts_id=customer.contacts_id')
                    ->join(C('DB_PREFIX').'onsiteservice_customer onsiteservice_customer ON onsiteservice_customer.onsiteservice_id = onsiteservice.id')
                    ->join(C('DB_PREFIX').'onsiteservice_answer onsiteservice_answer ON onsiteservice_answer.onsiteservice_customer_id = onsiteservice_customer.id AND (onsiteservice_answer.problem_id = 22 or onsiteservice_answer.problem_id = 39)')
                    ->where($where)->count();
                $page = new Page($count, $listrows);// 实例化分页类 传入总记录数和每页显示的记录数
                $page -> parameter = http_build_query($params);
                $show  = $page->show();// 分页显示输出
                $list = $mod->alias('onsiteservice')
                    ...
                    ->join(C('DB_PREFIX').'onsiteservice_customer onsiteservice_customer ON onsiteservice_customer.onsiteservice_id = onsiteservice.id')
                    ->join(C('DB_PREFIX').'onsiteservice_answer onsiteservice_answer ON onsiteservice_answer.onsiteservice_customer_id = onsiteservice_customer.id AND (onsiteservice_answer.problem_id = 22 or onsiteservice_answer.problem_id = 39)')
                    ->where($where)->page($p, $listrows)->order('remore_time desc')->select();
                $this->assign('page',$show);// 赋值分页输出
                $this -> assign('list',$list);
                $this -> display('visit');
                break;
...
//                $this->redirect('index') ;
    ...
    public function index(){
        ...
        $list = $mod->alias('onsiteservice')
            ...
            ->join(C('DB_PREFIX').'customer customer ON customer.customer_id=onsiteservice.customer_id')
            ->join(C('DB_PREFIX').'contacts contacts ON contacts.contacts_id=customer.contacts_id')
            ->page($p.','.$listrows)->order('addtime desc')->where($where)->select();
    ...
    public function addHomeDemand(){
        ...
        $m_onsiteservice = M('Onsiteservice');
        $position_id = I('session.position_id');
        $customer_id = I('get.customer_id') ;
        $platinum_ids = M('product')->where(['category_id' => ['in', '1,5,8,2']])->getField('product_id', true);//获取铂金版,黄金版,标准版产品id 添加经典版显示
        if (!empty($customer_id)){
            $customerData = $m_customer->alias('customer')
                ...
                ->where(['customer.customer_id'=>$customer_id, 'order_product.is_deleted'=>0, 'order_product.product_id'=>['in', $platinum_ids]])
        ...
        $not_the_door = $m_r_order_product->alias('order_product')
            ...
            ->field('customer.customer_id,customer.name customer_name,contacts.name contacts_name,contacts.telephone,order_product.id order_product_id,
            order_product.product_name,user.name owner_role_name,order_product.create_time,customer.address,user.role_id owner_role_id')
            ->where($where)
            ->where(['order_product.is_deleted'=>0, 'order_product.product_id'=>['in', $platinum_ids]])
            ->order('order_product.id desc')
            ->page($p, $listrows)
            ->select();
        $count = $m_r_order_product->alias('order_product')
            ...
            ->join(C('DB_PREFIX').'user user ON user.role_id = order_product.owner_role_id')
            ->where($where)
            ->where(['order_product.customer_id'=>['not in',$customer_ids], 'order_product.is_deleted'=>0, 'order_product.product_id'=>['in', $platinum_ids]])
            ->count();
        $Page = new Page($count, $listrows);
        $show = $Page -> show();
        $this->assign('not_the_door', $not_the_door);
        $this->assign('page',$show);
        $this->display();
    }
    ...
    public function insertHomeDemand(){
        ...
        $is_exist = (4 == $arr['type']) ? null: $mod -> where('type = %d AND customer_id = "%d"', $arr['type'], $arr['customer_id']) ->find();//判断是否之前是否有过提交, 多次上门的排除
        if (is_null($is_exist)){
            if ($arr['operator_name'] && $arr['status']==5) $arr['status'] = 1; //如果此时填了上门人直接将状态改为已安排（后面给财务添加了两个状态的添加）培训状态默认未未安排
            $list = $mod -> add($arr);
            if ($list) {
                if (!($arr['status'] == 4 || $arr['return_status'] == 3)){//财务可填写这两处，培训结束或回访结束不发短信
                    $message = new Message();
                    $message->sendMessageToTeacher(Message::SCHEDULED_TO_COMPLETE, $list); //调度完成
                }
                if (1 == I('get.is_next')){
                    alert('success','添加成功', U('Doorservice/schedu_update','id='.$list));
                }else{
                    if ($customer_id = I('get.customer_id_flag')){
                        alert('success','添加成功', U('customer/view','id='.$customer_id.$tab));
                    }elseif ($arr['status'] == 4 || $arr['return_status'] == 3){// 若培训结束或回访结束，跳到回访调查(search模块，默认是所有)
                        echo "<script>window.location.href='".U('Doorservice/huifang', 'id='.$list)."'</script>";
                    }else{
    ...
    public function edit(){
        ...
        if ($list!==false) {
            if ($list) {//确实修改了才执行发短信操作
                $message = new Message();
                $message->sendMessageToTeacher(Message::SCHEDULED_TO_COMPLETE, $id); // 调度完成
            }
            echo "<script>window.location.href='".U('Doorservice/index')."'</script>";
        }else{
            echo "<script>alert('提交失败');history.back();</script>";
    ...
    public function schedu_edit()
    {
        ...
        if (false !== $list) {
            ...
            if ($onsiteservice['order_product_id'] && (1 == $onsiteservice['type'])){// 安装前就是第一次上门时才发
            ...
            if (1 == I('get.is_next'))
                echo "<script>window.location.href='".U('Doorservice/teacher','id='.$id)."'</script>";
            else
                echo "<script>window.location.href='".U('Doorservice/scheduling')."'</script>";
        }else{
            echo "<script>alert('提交失败');history.back();</script>";
        }
    }
    ...
    public function huifangbianxie(){
        ...
        if ($list) {
            if (1 == I('get.is_next'))
                echo "<script>window.location.href='".U('Doorservice/verify_edit', 'id='.$id)."'</script>";
            else
                echo "<script>window.location.href='".U('Doorservice/visit')."'</script>";
        }else{
            echo "<script>alert('提交失败');history.back();</script>";
        }
    }
    ...
    public function verify_edit()
    {
        ...
        $list = $mod->alias('onsiteservice')
            ->join(C('DB_PREFIX').'onsiteservice_customer onsiteservice_customer ON onsiteservice_customer.onsiteservice_id = onsiteservice.id')
            // ->join(C('DB_PREFIX').'onsiteservice_answer onsiteservice_answer ON onsiteservice_answer.onsiteservice_customer_id = onsiteservice_customer.id AND onsiteservice_answer.problem_id = 22')
            ->field('onsiteservice.*,onsiteservice_customer.id onsiteservice_customer_id')
            ->where(['onsiteservice.id'=>$id])->find();
        $this -> assign('vo',$list);
    ...
    public function verify_update()
    {
        ...
        $list = $mod ->where("$id = id")-> save($arr);
        if ($list) {
            echo "<script>window.location.href='".U('Doorservice/verify')."'</script>";
        }else{
            echo "<script>alert('提交失败');history.back();</script>";
        }
    }
    ...
    public function teacherxiede(){
        ...
        if ($list!==false) {
            ...
            if (1 == I('get.is_next'))
                echo "<script>window.location.href='".U('Doorservice/huifang', 'id='.$id)."'</script>";
            else
                echo "<script>window.location.href='".U('Doorservice/thedoorserver')."'</script>";
        }else{
            echo "<script>alert('提交失败');history.back();</script>";
        }
    }
...
//            ->join(C('DB_PREFIX').'user user ON user.user_id = onsiteservice.tid')
    ...
    public function getCustomerAnswer()
    {
        ...
        $suggestion = M('onsiteservice_customer')->where(['onsiteservice_id'=>$id])->field('suggestion,create_time')->find();
    ...
    public function theDoorServer(){
        ...
        $list = $mod->alias('onsiteservice')
            ...
            ->join(C('DB_PREFIX').'customer customer ON customer.customer_id=onsiteservice.customer_id')
            ->join(C('DB_PREFIX').'contacts contacts ON contacts.contacts_id=customer.contacts_id')
            -> page($p, $listrows) ->order('timing desc,addtime desc')->where($where)-> select();
        $this -> assign('page',$show);
        $this -> assign('list',$list);
        $this->display();
    }
    ...
    public function visit(){
        ...
        $list = $mod->alias('onsiteservice')
            ...
            ->join(C('DB_PREFIX').'onsiteservice_customer onsiteservice_customer ON onsiteservice_customer.onsiteservice_id = onsiteservice.id')
            ->join(C('DB_PREFIX').'onsiteservice_answer onsiteservice_answer ON onsiteservice_answer.onsiteservice_customer_id = onsiteservice_customer.id AND (onsiteservice_answer.problem_id = 22 or onsiteservice_answer.problem_id = 39)')
            ->field('onsiteservice.*,onsiteservice_answer.answer,customer.name customer_name,contacts.name contacts_name, contacts.telephone contacts_telephone')
            -> page($p, $listrows)->where($where)->order('remore_time desc')-> select();

        // $sms_record = M('sms_record');
        // foreach ($list as &$value){
        //     $shortUrl = \App\Extend\Http::urlLongToShort('http://mcrm.lyfz.net/index.php/doorservice/getProblemList?id='.$value['id'])[0]['url_short'];
        //     $value['sms_count'] = $sms_record->where(['to_customer_id'=>$value['customer_id'],'content'=>'感谢您对利亚方舟培训工作的支持与配合。为更好地提升利亚方舟给您的服务，请对本次上门培训给予评价（我们非常重视您的看法，请如实填写，谢谢您的支持）'.$shortUrl])->count();
        // }
        $this -> assign('page',$show);
        $this -> assign('list',$list);
        $this->display();
...
//                     ->where($map)
    ...
    public function sendMessageCron()
    {
        ...
        $onSiteServiceArr = $onSiteService->alias('onsiteservice')
            ->field('customer_id,remore_time,type')
...
//            ->join(C('DB_PREFIX').'receivingorder receivingorder ON receivingorder.receivables_id = receivables.receivables_id')
            ->where('onsiteservice.remore_time <> "" AND unix_timestamp(now()) < UNIX_TIMESTAMP(remore_time)+86400*61')
            ->select();
    ...
    {
        $customerInfo = M('onsiteservice')->alias('onsiteservice')
            ->join(C('DB_PREFIX').'customer customer ON customer.customer_id = onsiteservice.customer_id')
            ->join(C('DB_PREFIX').'contacts contacts ON contacts.contacts_id = customer.contacts_id')
            ->where('onsiteservice.id = %d', $id)
            ->field('onsiteservice.id onsiteservice_id,customer.name customer_name,customer.address,contacts.telephone,onsiteservice.type')
            ->find();
        $genre = ($customerInfo['type']==1) ? 1 : 2;
        $problems = M('OnsiteserviceProblem')->where(['status'=>1, 'genre'=>$genre])->order('description')->select();

        $adjustmentProblem = [];
        $genreDescription =[
            '',
            ['','一、培训流程执行','二、软件功能执行','三、软件维护操作','四、满意度调查'],
            ['','一、重新上门辅导原因（多选项）','二、以下功能启用情况','三、回访前存在的问题老师培训后是否解决？','四、满意度调查']
        ];
        foreach ($problems as $problem){
            $adjustmentProblem[$genreDescription[$genre][$problem['description']]][] = [
            ...
            $adjustmentProblem[$genreDescription[$genre][$problem['description']]]['type'] = (int) $problem['type'];
            $adjustmentProblem[$genreDescription[$genre][$problem['description']]]['answer_total'] = (int) count(explode(',', $problem['answer']));
        }
        // var_dump($adjustmentProblem);die;
        $this->assign('adjustmentProblem', $adjustmentProblem);
        $this->assign('customerInfo', $customerInfo);
        $this->display('survey'.$genre);
    }

...
class SettingAction extends Action{
	...
	public function statusflowEdit(){
		if ($this->isGet()) {
			$flow = M('BusinessStatusFlow')->where("flow_id =%s", $_GET['id'])->find();
			$this->flow = $flow;
    ...
    public function sendSms(){
        ...
        $current_sms_num = $messageObj->queryBalance()['data']['balance'];
        $messageAccount = $messageObj->getAccount();
        if($this->isPost()){
            ...
            if(!$messageAccount) alert('success',L('SEND_SMS_FAILED'),$_SERVER['HTTP_REFERER']);
            ...
            $success_array = array();
            if($phoneNum && $message){
                if(strpos($message,'{name}',0) === false){//模板中没有{name}
    ...
    //                var_dump($fail_message);die;
                    //echo '发送成功!';die();
    //                $result = sendGroupSMS(implode(',', $success_array),$message,'sign_name', $sendtime);
                    $result = $messageObj->sendMessage(implode(',', $success_array),$message);
                    if ($result['code']<0){
                        alert('error',L('SMS_NOTIFICATION_FAILS_CODE', $result['info']),$_SERVER['HTTP_REFERER']);
                    }
                    if($result['data']['code'] == 0){
                        ...
                        alert('error',L('SMS_NOTIFICATION_FAILS_CODE', $result['data']['code']),$_SERVER['HTTP_REFERER']);
                    }
                }else{
                    foreach($phone_array as $k=>$v){
    ...
    //                            var_dump($phone);
    //                            var_dump($info_array[1]);die;
    //                            $result =sendSMS($phone, $real_message, 'sign_name', $sendtime);
                                $result = $messageObj->sendMessage($phone, $real_message);
                                ...
                                $data['sendtime']=time();
                                $data['to_contacts_name']=$info_array[1];
                                $m_sms_record->add($data);
                            }else{
                                $fail_array[] = $v;
                            }
...
//			$params = array('field='.$field, 'condition='.$condition, 'search='.trim($_REQUEST["search"]));
            $params = I('get.');
            if	('sendtime' == $field) $where[$field] = ['between', [I('get.start_time', null ,'strtotime'), I('get.end_time', null, 'strtotime')+86399]];
        }
	    $p = isset($_GET['p']) ? intval($_GET['p']) : 1 ;
		// if(!session('?admin')){
		// 	$where['role_id'] = session('role_id');
		// }
        $where['role_id'] = ['in', getSubRoleId()];
		...
		$list = $m_sms_record->where($where)->page($p.','.$listrows)->order('sms_record_id desc')->select();
		foreach($list as $k=>$v){
			//查发送人名字
			...
			//截取内容
//			if(mb_strlen($v['content'],'utf-8') >= 100){
//				$list[$k]['subcontent'] = mb_substr($v['content'],0,100,'utf8').'...';
//			}else{
...
//			}
	...
	**/
	...
	public function sendemail(){
		if($this->isPost()){
			if(!$smtp = M('UserSmtp')->where('smtp_id = %d', intval($_POST['smtp']))->find()){
				if(!$m = M('config')->where('name = "smtp"')->find()){
    ...
    {
        $product_list = M('product')->getField('product_id,name');
        $this->assign('product_list', $product_list);
        if ($this->isPost()){
            $config = M('config');
            $posts = I('post.');
            $bool1 = $config->where('name = "%s"', "order_font_family")->save(['value'=>$posts['font_family']]);
            $bool2 = $config->where('name = "%s"', "not_display_product_id")->save(['value'=>serialize($posts['select_products'])]);
            $config->where('name = "%s"', "return_deadline")->save(['value'=>$posts['return_deadline']]);
            $config->where('name = "%s"', "review_deadline")->save(['value'=>$posts['review_deadline']]);
            $config->where('name = "%s"', "overdue_return")->save(['value'=>$posts['overdue_return']]);
            if ($bool1!==false && $bool2!==false){
                $this->ajaxReturn(['code' => 1, "info" => "保存成功"]);
            }else{
                $this->ajaxReturn(['code' => -1, "info" => "保存失败"]);
            }
		}
	...
	public function renderModulesTpl($module) {
		$tmp_file_path = APP_PATH . "Modules/$module/views/pages/settings.html";
		if ( file_exists($tmp_file_path) ){
			$this->display($tmp_file_path);
		}
	}
}
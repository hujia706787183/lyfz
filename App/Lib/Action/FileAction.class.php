	...
	public function add(){
		...
		if($_POST['submit']){
			if (array_sum($_FILES['file']['size'])) {
				...
				alert('error','请选择你要上传的文件！',$_SERVER['HTTP_REFERER'].$tab);
			}
			$m_file = M('File');
			...
			foreach($info as $key=>$value){
				...
				if($file_id = $m_file->add($data)){
					...
					if(0 >= $r_file_module->add($temp)){
						alert('error', L('ADD_FAILURE_PARTS_ACCESSORIES'), $_SERVER['HTTP_REFERER'].$tab);
					}
				}else{
					alert('error', L('ADD_ATTACHMENTS_FAIL'), $_SERVER['HTTP_REFERER'].$tab);
				};
			}
			alert('success',L('ADD_ATTACHMENTS_SUCCESS'), $_SERVER['HTTP_REFERER'].$tab);
...
//	}
	public function delete(){
        ...
        $tab = I('cookie.tab','') ;
		if (0 == $file_id){
			alert('error',L('PARAMETER_ERROR'),$_SERVER['HTTP_REFERER'].$tab);
		}else{
			if (isset($_GET['r']) && isset($_GET['id'])) {
				...
				if (is_array($file)){
					if ($m_r->where('file_id = %d',$_GET['id'])->delete()) {
						if ($m_file->where('file_id = %d',$_GET['id'])->delete()) {
							alert('success',L('DELETED SUCCESSFULLY'),$_SERVER['HTTP_REFERER'].$tab);
						}else{
							alert('success',L('DELETE FAILED CONTACT THE ADMINISTRATOR'),$_SERVER['HTTP_REFERER'].$tab);
						}
					}else {
						alert('success',L('DELETE FAILED CONTACT THE ADMINISTRATOR'),$_SERVER['HTTP_REFERER'].$tab);
					}
				} else {
					alert('success',L('YOU_HAVE_NO_RIGHT_TO_DELETE_THE_ATTACHMENT'),$_SERVER['HTTP_REFERER'].$tab);
				}
				...
				if (is_array($file)) {
					if($m_file->where('file_id = %d', $_GET['id'])->delete()){
						alert('success', L('OPERATION_IS_SUCCESSFUL'), $_SERVER['HTTP_REFERER'].$tab);
					}
				} else {
					alert('error', L('YOU_HAVE_NO_RIGHT_TO_DELETE_THE_ATTACHMENT'), $_SERVER['HTTP_REFERER'].$tab);
				}
			}
		}
	}

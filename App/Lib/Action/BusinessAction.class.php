...
class BusinessAction extends Action{
	...
	**/
	public function _initialize(){
		$action = array(
			'permission'=>array(''),
			'allow'=>array('close','analytics','getaddchartbyroleid','changecontent','getownchartbyroleid','advance','validate','check','revert','getsalesfunnel','getcurrentstatus')
		);
	...
	**/
	public function validate() {
		if($this->isAjax()){
            ...
            $field = M('Fields')->where('model = "Business" and field = "%s"', $this->_request('clientid','trim'))->find();
	...
	**/
	public function index(){
        ...
        // 二次开发修改支持影楼名称模糊搜索
        if($_GET['field'] == 'customer_id'){
            $customer = M('customer');
            $customer_map['name'] = array('like','%'.I('get.search').'%');
            $customer_res = $customer->where($customer_map)->field('customer_id')->select();
            $customer_list = array_column($customer_res, 'customer_id');
            $where['customer_id'] = array('in',$customer_list);
        }
        
        // 二次开发修改客户状态判断
        $config = M('config');
        $config_map['name'] = 'business_status_id_list';
        $config_res = $config->where($config_map)->find();
        $where['status_id'] = array('in',explode(',',$config_res['value']));
	...
	**/
	public function excelExport($businessList=false){
        ...
        header("Content-Disposition:attachment;filename=5kcrm_business_".date('Y-m-d',mktime())."_".$current_page.".xls");
	...
	public function advance(){
		if($this->isPost()){
			...
			if($m_business->where('business_id = %d', $id)->save($data2)){
				...
				// 		'update_time' => time(),
				// 		'status' => 0,
				// 		'price' => $business['estimate_price'],
				// 		'owner_role_id' => $business['owner_role_id'],
				// 		'creator_role_id' => session('role_id')
				// 	];
				// 	$receivables->add($data);
				// };
			...
			if($id > 0){
				...
				$order_id_min = M('BusinessStatus')->where('status_id = %d', $status_id)->getField('order_id');
				if(!$order_id_min) $order_id_min = 0;
				$order_id_max = $order_id_min < 4 ? 4 : 100 ;
				$business_status_conditios = [
					'order_id' => [
						'BETWEEN',
						[(Int)$order_id_min,$order_id_max]
					]
				];
				$statusList =  M('BusinessStatus')->where($business_status_conditios)->order('order_id')->select();

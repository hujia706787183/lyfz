...
function actionLog($id,$param_name='',$text=''){
    ...
    if($action_name == 'addnew' || $action_name == 'mylog_add'){
        $data['action_name'] = 'add';
    }elseif($action_name == 'editnew'){
        $data['action_name'] = 'edit';
    }elseif($action_name == 'viewnew'){
    ...
    $data['action_id'] = $id;
    $data['content'] = L('ACTIONLOG',array($category,$user['name'],date('Y-m-d H:i:s'),L($data['action_name']),$id,L(MODULE_NAME),$text));
    $actionLog = M('actionLog');
    $actionLog->create($data);
    if($actionLog->add()){
        //如果是删除操作，设置action_delete = 1
        if(strtolower($data['action_name']) == 'delete' || strtolower($data['action_name']) == 'completedelete' || strtolower($data['action_name']) == 'log_delete'){
...
// 不包括自己所在部门
function getSubDepartment($department_id, $department, $separate = null, $no_separater = null) {
    $array = array();
    if($no_separater){
...
function getSubRole($role_id, $role_list, $separate = '') {
    ...
    /** 35 为离职员工的部门 */
    $son_role_item_list = $d_role->where(['role.position_id' => ['in', array_diff(array_column($sub_position, 'position_id'), ['35'])]])->select();
    $son_role_list = [];
    foreach($son_role_item_list as $son_role_item){
        $son_role_list[$son_role_item['position_id']][] = $son_role_item;
    }

    foreach($sub_position as $position_id) {
        // $son_role = $d_role->where('role.position_id = %d', $position_id['position_id'])->select();
        $son_role = $son_role_list[$position_id['position_id']];
        if ($son_role)
            foreach($son_role as $val){
                $array[] = [
                    ...
                    'name' => $separate . $val['department_name'] . ' | ' . $val['role_name']
                ];
            }
    }
    return $array;
...
function getSubPosition($position_id, $position, $separate=null) {
    $array = array();
...
function sendRequest($url, $params = array() , $headers = array()) {
    return Http::sendRequest($url, $params, $headers);
}
...
function bsendemail($address,$title,$content,$file=array(),$author=false,$selfsmtp=false){
    ...
    if($selfsmtp){
        ...
        if(!$smtp && $selfsmtp != '-1'){
            $smtp = M('config')->where('name = "smtp"')->find();
            C(unserialize($smtp['value']), 'smtp');
        }
        ...
        if ($v['field'] == 'customer_id') {
            ...
            switch ($v['form_type']) {
                ...
                case 'datetime' :
                    ...
                    $field_list[$k]['html'] = '<input  onFocus="WdatePicker({dateFmt:\''.$time_accuracy.'\'})" name="'.$v['field'].'" id="'.$v['field'].'" type="text" value="'.pregtime($value).'"/> &nbsp;  <span id="'.$v['field'].'Tip" style="color:red;"></span>'.$input_tips;
                    break;
                case 'number' :
                    $field_list[$k]['html'] = '<input type="text" data-type="nummber"  id="'.$v['field'].'" name="'.$v['field'].'" maxlength="'.$v['maxlength'].'" value="'.$value.'"/> &nbsp;  <span id="'.$v['field'].'Tip" style="color:red;"></span>'.$input_tips;
                    break;
                case 'floatnumber' :
                    $value = $value > 0 ? $value : ''; 
...
function check_permission($module_id, $module, $permission_field='owner_role_id'){
    ...
    $role_id = intval(session('role_id'));
    if($module == 'customer'){
        if (D('Customer')->isOrdered($module_id)){
            return true;
        }
        $owner_role_id = M($module)->where($module.'_id = %d', $module_id)->getField('owner_role_id');
        $sharing_role_ids = M('CustomerShare')->where('customer_id = %d',$module_id)->getField('by_sharing_id',true);
    }else{
        $owner_role_id = M($module)->where($module.'_id = %d', $module_id)->getField($permission_field);
    }
    $permission_ids = getSubRoleId();
    ...
    if(!$owner_role_id 
        || in_array($owner_role_id, $permission_ids)
        || in_array($role_id, $sharing_role_ids)){
        return true;
    }else{
        return false;
    } 
...
function getCheckUrlByAction($m, $a){
    switch (strtolower($a)) {
        ...
        case "salesreturn" : $a = 'index'; break;
        case "addsalesreturn" : $a = 'add'; break;
        case "editsalesreturn" : $a = 'edit'; break;
...
/**
 * @ atuhor     : zf
 * @ function   : 获取权限范围内roleid
 * @ return     : 返回roleid的数组
 * @ des        : 参数说明：
 *                $module 对应的模块名 小写
 *                $action 对应的方法名
 *                $sub_role 为true的话 返回权限范围内的下属roleid
 *                          为false 返回权限范围内的roleid
 **/
 ...
 * author : xiaoxu
 * function : app列表权限返回
 * param    : $m=MODULE_NAME $role_id 岗位ID 
 **/
function permissionlist($m,$role_id){
    if(session('?admin')){
        $permission = array();
        $permission['edit'] = 1;
        $permission['view'] = 1;
        ...
        foreach($list as $k=>$v){
            if($v == 'view'){
                $all_ids = getPerByAction($m,'view');
                if(in_array($role_id, $all_ids)){
                    $permission['view'] = 1;
                }
            }elseif($v == 'edit'){
                $all_ids = getPerByAction($m,'edit');
                if(in_array($role_id, $all_ids)){
                    $permission['edit'] = 1;
                }
            }elseif($v == 'delete'){
                $all_ids = getPerByAction($m,'delete');
                if(in_array($role_id, $all_ids)){
                    $permission['delete'] = 1;
                }
            }
...
function getLastedImg(){
    $imgsrc = F('img_id_'.session('user_id'));
    if(!$imgsrc){
        $imgsrc = M('User')->where('user_id = %d', intval(session('user_id')))->getField('img');
    }
    if($imgsrc){
        return $imgsrc;
    }else{
...
/**
 * 图片保存处理
 * @param array $img_arr 图片base64 list
 * @return obj code 200成功
 */
function imgHandle($img_arr){
    //图片数据处理
    if(count($img_arr) == 0){
        $res->meta->code = 400;
        $res->meta->message = '至少上传一张图片';
        return $res;
    }
    
    foreach($img_arr as $k=>$v){
        $file_name = RandCode(16);
        
        //判断图片文件后缀名类型
        switch(explode('/',explode(';',$v)[0])[1]){
            case 'png':
                $extension = '.png';
                break;
            ...
            default:
                $res->meta->code = 4111;
                $res->meta->message = '您上传的图片不支持!';
                return $res;
        }

        //生成保存路径
        $time = date("Ymd");
        $path = './Uploads/img/'.$time;
        mkdir($path,0777);
        $file_route = './Uploads/img/'.$time.'/'.$file_name.$extension;
        $img_url[] = '/Uploads/img/'.$time.'/'.$file_name.$extension;

        //文件写入磁盘
        $img = base64_decode(substr(explode(';',$v)[1],7),22);
        $size = file_put_contents($file_route,$img);

        //判断文件大小
        if($size > 3048576){
            unlink($file_route);
            $res->meta->code = 400;
            $res->meta->message = '图片超过3M';
            return $res;
        }
    }
...
function event(string $eventName, ...$params){
    $modules = App\Common\Modules::list();

    foreach ($modules as $module){
        if (!empty($module['events'])) {
            foreach ($module['events'][$eventName] as $eventHandlerName) {
                $eventHandlerStr = 'App\Modules\\'.$module['key'].'\Events\\'.$eventHandlerName;
                $eventHandler = new $eventHandlerStr();
...
function stylesheet($staticId,  $path = ''){
    // 逻辑比较多,暂时简单一点
    $map = [
        'common.css' => '/App/Modules/' . MODULE_NAME . '/statics/common.css',
        'mint-ui' => 'https://cdn.bootcss.com/mint-ui/2.2.13/style.css',
        'element-ui' => 'https://cdn.bootcss.com/element-ui/2.3.3/theme-chalk/index.css'
    ];

    $fileuri = $map[$staticId];

    if (!empty($fileuri)){
        return '<link href="'. $fileuri .'" rel="stylesheet">';
    }
    
}

function javascript(...$staticIds){
    // 逻辑比较多,暂时简单一点
    $map = [
        'vue:2.5.16' => 'https://cdn.bootcss.com/vue/2.5.16/vue.min.js',
        'element-ui:2.2.2' => 'https://cdn.bootcss.com/element-ui/2.2.2/index.js',
        'axios:0.18.0' => 'https://cdn.bootcss.com/axios/0.18.0/axios.min.js',
        'jquery-cookie:1.4.1' => 'https://cdn.bootcss.com/jquery-cookie/1.4.1/jquery.cookie.min.js',
        'lodash:4.17.5' => 'https://cdn.bootcss.com/lodash.js/4.17.5/lodash.min.js'
    ];
    ...
    foreach ($staticIds as $staticId){
        ...
        if (!empty($fileuri)){
            $staticHtml .= '<script src="'. $fileuri .'"></script>' . "\n";
        }
    }
    return $staticHtml;
}

require_once("./App/Common/Http.class.php");
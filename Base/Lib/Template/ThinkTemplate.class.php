...
class  ThinkTemplate {
    ...
    protected function compiler($tmplContent) {
        ...
        // 还原被替换的Literal标签
        $tmplContent =  preg_replace_callback('/<!--###literal(\d+)###-->/is', function($matches){
    ...
    public function parse($content) {
        ...
        //解析普通模板标签 {tagName}
        $content = preg_replace_callback('/('.$this->config['tmpl_begin'].')([^\d\s'.$this->config['tmpl_begin'].$this->config['tmpl_end'].'].+?)('.$this->config['tmpl_end'].')/is',
        function($matches){
        ...
        foreach ($tLib->getTags() as $name=>$val){
            ...
            foreach ($tags as $tag){
                ...
                if (!$closeTag){
                    ...
                    $replacement    = function ($matches) use ($tagLib, $tag) {
                        return $this->parseXmlTag($tagLib, $tag, $matches[1], '');
                    };
    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            ...
            /** 增加modules支持 */
            if (!file_exists_case($templateName)){
                $templateName  = './App/Modules/'.$module.'/views/'.$action.$this->config['template_suffix'];
            }
            // 获取模板文件内容

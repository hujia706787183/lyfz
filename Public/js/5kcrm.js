function changeCondition(){
	...
	if(a == 'number') {
		...
		$searchContent.html('<input type="text" class="input-medium search-query" name="start_time" onclick="WdatePicker()"/> - \
			<input type="text" class="input-medium search-query" name="end_time" onclick="WdatePicker()"/>');
		var now = new Date();
		$searchContent.find('input').val(now.getFullYear() + '-' + _.padStart(now.getMonth() + 1, 2, 0) + '-' + _.padStart(now.getDate(), 2, 0));
	} else if (a == 'bool') {
		$("#conditionContent").html('<select id="condition" style="width:auto" name="condition" onchange="changeSearch()">'
        ...
        $.ajax({
			type: 'get',
			url: 'index.php?m=user&a=getDepartmentList',
			async: false,
			success: function (data) {
				options = '<option class="all" value="all">全部</option>';
				$.each(data.data, function (k, v) {
					options += '<option value="'+ v.department_id +'">'+ v.name +'</option> '
                });
                $("#searchContent").html('<select style="width:auto" name="department" id="department">'+ options +'</select>--<select style="width:auto" name="search" id="staffList"></select>');
                $("#conditionContent").html('');
            }
        });
        ...
        $.ajax({
            type: 'get',
            url: 'index.php?m=finance&a=getcostitem&level=2',
            async: false,
            success: function (data) {
                options = '';
                $.each(data.data, function (k, v) {
                    options += '<option value="' + v.cost_item_id + '">' + v.cost_item + '</option>';
                });
                $("#searchContent").html('<select id="search" style="width:auto" name="search">' + options + '</select>&nbsp;&nbsp;');
                $("#conditionContent").html('');
            },
        ...
        $("#searchContent").html('<select name="state" id="state" style="width:80px;margin-bottom:10px;"></select>&nbsp;'
							+'<select name="city" id="city" style="width:80px;margin-bottom:10px;"></select>&nbsp;'
							+'<select name="area" id="area" style="width:80px;margin-bottom:10px;"></select>&nbsp;'
							+'<input type="text" id="search" style="width:100px;margin-bottom:10px;" name="search" placeholder='+CrmLang.STREET_INFORMATION+' class="input-large">&nbsp;&nbsp;');
        new PCAS("state","city","area","","","");
	} else if (a == 'dept') {
        $.ajax({
            type: 'get',
            url: 'index.php?m=finance&a=getcostitem&level=1',
            async: false,
            success: function (data) {
                options = '';
                $.each(data.data, function (k, v) {
                    options += '<option value="' + v.cost_item + '">' + v.cost_item + '</option>';
                });
                $("#searchContent").html('<select id="search" style="width:auto" name="search">' + options + '</select>&nbsp;&nbsp;');
                $("#conditionContent").html('');
            },
    ...
    } else if (a == 'product_name') {
        $.ajax({
            type:'get',
            url:'index.php?m=product&a=getProductList',
            async:false,
            success:function(data){
                options = '';
                ...
                $("#conditionContent").html('<label for="field">产品名:</label><select style="width:auto" name="condition" id="condition" class="form-control">' + options + '</select>&nbsp;&nbsp;');
            },
            dataType:'json'
        });
	} else if (a == 'product_type') {
        $.ajax({
            type:'get',
            url:'index.php?m=product&a=getProductType',
            async:false,
            success:function(data){
                options = '';
                $.each(data, function(k, v){
                ...
                $("#conditionContent").html('<label for="category_id">产品类型:</label><select style="width:auto" name="condition" id="condition" class="form-control">' + options + '</select>&nbsp;&nbsp;');
            },
            dataType:'json'
        });
    }
